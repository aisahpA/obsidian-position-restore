...
export const zh: En = {
	...
	// 的都交给下面各行自己说。
	'recentFiles.intro':
		'一份去处的清单：最近打开过的笔记，主区域里没有文件的视图（关系图谱这类）也各占一行；点一行就回到那一处。要不要连你在笔记里跳转过的标题与块一起记，由下面那条开关决定（默认不记）。它不是「前进与后退」页那份历史——那里记的是怎么走到这里，这里记的是去过哪些地方，两者各记各的。清单只存在本机，不随仓库同步；重启后仍在。',
