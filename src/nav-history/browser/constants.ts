...
// under: the small print about the step, the section, and the bar with the travel button
// and the view switch — measured at 85px on a phone with the two controls on ONE line
// (see styles.css), plus the slack a theme's own line-height can add. The row alone
// being on screen is not enough — the button a finger travels with is in the panel, and
// the panel as a whole cannot be fitted, because it is taller than the list it sits in.
...
// on a pointing device, where its pinned head keeps the travel button and the view
// switch in reach whatever the note's length.
