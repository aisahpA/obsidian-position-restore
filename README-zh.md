...
- **最近文件** — 你去过的笔记，以及你在其中点过的小标题与锚点链接；同一份列表也可以常驻在侧边栏，左侧 ribbon 上也有一个入口（桌面端打开常驻侧栏，手机端打开对话框；不需要它，可以在 ribbon 里把它取消勾选）
...
- **导航历史** — VSCode 风格的前进/后退，覆盖文件切换、标签页切换和文件内跳转（大纲点击、锚点链接、光标跳变、关系图谱）；以后退 / 前进两个命令提供，可绑定快捷键
- **最近文件** — 「打开最近文件」是你去过的笔记的清单，按最近访问排序（主区域里没有文件的视图——关系图谱这类——也各占一行，行上带着这个视图自己的名字与图标；它离开时的状态也一并记下，所以标签页早就关掉之后点回去，局部图谱仍指向当时那篇笔记，插件视图仍是你离开时的过滤条件）。列表**记到多细**是一档设置：只记笔记、记落点但每篇仍是一行（落点只用来搜索）、每个落点各占一行（默认）；往回收只是不再记新的，已经记下的会留着，直到那篇笔记被挤出列表。它是独立的一份数据，不是前进后退那个栈：回退之后再跳去别处只是新增一个地点，不会把前面那一截丢掉；它也有自己的一份「不收录的文件夹」和「不收录的属性」，所以一个你不想记录位置的文件夹、或一类按 frontmatter 排除掉的笔记，仍然可以是你想回头打开的地方。点笔记那一行是**普通打开**——落点由位置记录决定，和你从资源管理器点开完全一致；点小标题/锚点那一行才落到那个具体位置。列表记住多少篇笔记是同一页上的另一项。把指针停在一行上，会向 Obsidian 要它自己的页面预览，开的是这一行代表的东西：笔记那一行给的是**整篇**，滚到你点开它会落的那一处；小标题那一行给的是那一节。而且它是**一出现就站在那里**，不是先显示开头再跳过去（只把行号交给原生预览时它是会跳的）。默认需要**按住 Ctrl/Cmd** 才会出现，这个面板按名字列在「核心插件 → 页面预览」里，初始值与 app 自己的几份列表一致（要按住键），改也在那里改。
- **最近文件侧栏** — 「在侧边栏打开最近文件」把同一份列表变成常驻面板：放在哪儿就留在哪儿，随列表变化实时更新，跳转之后面板不会关闭。列表本身就是导航：点一行就打开它代表的那处——一行一个手势，没有第二半。列表默认每个落点各占一行，直接列在笔记下面，带着行号和所属章节；同一档设置也可以停在「每篇一行」——那时笔记那一行代表你最后离开的那一处，更早的落点用搜索框找（搜索框匹配的是当时记录的上下文，所以「那几行话」仍然找得回来）——或者干脆只记笔记。
