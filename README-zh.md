...
- **最近文件侧栏** — 「在侧边栏打开最近文件」把同一份列表变成常驻面板：放在哪儿就留在哪儿，随列表变化实时更新，跳转之后面板不会关闭。列表本身就是导航：点一行就打开它代表的那处。行首那个控件是这一行的另一半——它把这一行的详情显示在旁边的面板里（当时记录的落点，或这篇笔记现在的样子），自己不打开任何东西，再点一下收起；这一栏默认不开，齿轮里第一项「落点详情 · 显示」把它打开，打开后行首才出现那个控件。面板只认明确的点击（对话框里的那份列表也一样），鼠标划过不会有任何反应。列表默认每篇笔记一行，更早的落点用搜索框找，或把工具栏最右端的设置切到「全部落点」让它们直接列在笔记下面——靠得很近的落点会并成一行。面板显示哪一份内容，是同一个齿轮里紧接着的那项设置，选过就记住。面板正文里的双链可以直接点，不会再让 WebView 自己跳走
