...
- **历史侧栏** — 「在侧边栏打开导航历史」把同一份列表变成常驻面板：放在哪儿就留在哪儿，随历史变化实时更新，跳转之后面板不会关闭；面板只认明确的点击（对话框里的那份列表也一样），鼠标划过不会有任何反应；跳转是单击行首的箭头，或在行上直接右键——按下去就走，中间不再有菜单要确认。一个文件默认只打印你离开时所在的那个落点，更早的落点在该行上点一下即可展开（设置里可选「全部」）；面板显示的是当时记录的落点，还是这篇笔记现在的样子——一个开关，选过就记住。面板正文里的双链可以直接点，不会再让 WebView 自己跳走
