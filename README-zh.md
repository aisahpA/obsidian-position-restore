...
- **最近文件** — 「打开最近文件」是「你去过的笔记 + 你在其中点过的小标题/锚点链接」，按最近访问排序（主区域里没有文件的视图——关系图谱、Thino 这类——也各占一行）。它是独立的一份数据，不是前进后退那个栈：回退之后再跳去别处只是新增一个地点，不会把前面那一截丢掉；它也有自己的一份「不收录的文件夹」，所以一个你不想记录位置的文件夹，仍然可以是你想回头打开的地方。点笔记那一行是**普通打开**——落点由位置记录决定，和你从资源管理器点开完全一致；点小标题/锚点那一行才落到那个具体位置。列表保留多少由工具栏齿轮里的最后一项决定。
