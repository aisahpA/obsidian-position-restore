...
- **最近文件** — 「打开最近文件」是「你去过的笔记 + 你在其中点过的小标题/锚点链接」，按最近访问排序。它是独立的一份数据，不是前进后退那个栈：回退之后再跳去别处只是新增一个地点，不会把前面那一截丢掉；它也有自己的一份「不收录的文件夹」，所以一个你不想记录位置的文件夹，仍然可以是你想回头打开的地方。点笔记那一行是**普通打开**——落点由位置记录决定，和你从资源管理器点开完全一致；点小标题/锚点那一行才落到那个具体位置。列表保留多少由工具栏齿轮里的最后一项决定。
- **最近文件侧栏** — 「在侧边栏打开最近文件」把同一份列表变成常驻面板：放在哪儿就留在哪儿，随列表变化实时更新，跳转之后面板不会关闭。列表本身就是导航：点一行就打开它代表的那处——一行一个手势，没有第二半。列表默认每篇笔记一行（打开展开它代表的最后落点）；更早的落点用搜索框找（搜索框匹配的是当时记录的上下文，所以「那几行话」仍然找得回来），或把工具栏最右端的设置切到「全部落点」，让每个落点作为一行直接列在笔记下面，显示行号和所属章节——靠得很近的落点会并成一行。列表只认明确的点击（对话框里的那份列表也一样），鼠标划过不会有任何反应。
